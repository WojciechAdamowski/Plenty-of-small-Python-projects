"""
Oczko
Klasyczna gra karciana. (Ta gra nie uwzględnia rozdwojenia kart ani ubezpieczenia).
    ...
    while True:
        ...
        while True:
            ...
            if move == 'P':
                ...
                print('Zakład zwiększony do kwoty {0}.'.format(bet))
                print('Zakład: {0}'.format(bet))
            ...
            if move in ('D','P'):
                ...
                print('Wziąłeś {0} {1}.'.format(rank, suit))
...
def getBet(maxBet):
    while True:
        print('Ile chcesz postawić? (1-{0} lub KONIEC)'.format(maxBet))
...
def displayHands(playerHand, dealerHand, showDealerHand):
    ...
    if showDealerHand:
        print('KRUPIER: {0}'.format(dealerHand))
        displayCards(dealerHand)
    else:
        print('KRUPIER: ???')
        displayCards([BACKSIDE] + dealerHand[1:])

    print('GRACZ: {0}'.format(getHandValue(playerHand)))
